...
/*   Updated: 2022/08/29 11:13:38 by ntan-wan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	propagate_constraint(t_cell **board)
{
	...
	{	
		if ((*board)[i].size == 1)
		{
			...
			removed += remove_duplicated_num(board, i, row, (*board)[i].nums[j]);
			removed += remove_duplicated_num(board, i, col, (*board)[i].nums[j]);
			free(row);
			free(col);
		}
	}
